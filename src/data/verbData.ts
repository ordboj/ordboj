...
export interface VerbData {
  ...
  grupp?: Grupp;
  // True only for verbs that have NO natural imperativ in Swedish (modal
  // and auxiliary verbs: kunna, få, vilja, ...). For these rows imperativ
  // is deliberately the empty string. This flag is what makes that empty
  // string mean "this form does not exist" rather than "not filled in
  // yet" — an empty imperativ with noImperativ left undefined means the
  // data is missing and still needs to be entered, never that the form
  // doesn't exist.
  noImperativ?: boolean;
}
...
export const VERB_DATA: VerbData[] = [
  ...
  { cefr: "A1", infinitive: "kunna", imperativ: "", presens: "kan", preteritum: "kunde", supinum: "kunnat", grupp: "4", noImperativ: true },
  { cefr: "A1", infinitive: "unna", imperativ: "unna", presens: "unnar", preteritum: "unnade", supinum: "unnat", grupp: "1" },
  { cefr: "A1", infinitive: "få", imperativ: "", presens: "får", preteritum: "fick", supinum: "fått", grupp: "4", noImperativ: true },
  ...
  { cefr: "A1", infinitive: "vilja", imperativ: "", presens: "vill", preteritum: "ville", supinum: "velat", grupp: "4", noImperativ: true },
  ...
  // "te sig" is reflexive: the pronoun must agree with the addressee
  // (imperativ "te dig", not "te sig"), unlike a true particle verb where
  // the particle never changes. Left unfilled pending a linguist decision
  // on how reflexive pronoun agreement should be represented in this
  // single-string imperativ field — needs human check, not a guess.
