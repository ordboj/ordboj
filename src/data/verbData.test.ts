...
function rowsWithPrecedingComments(source: string): Array<{
  ...
  for (const line of lines) {
    ...
    if (rowMatch) {
      ...
      // rowMatch[1] is the regex's capture group: present whenever the row
      // matched, but string | undefined under noUncheckedIndexedAccess.
      const infinitive = rowMatch[1];
      if (infinitive === undefined) continue;
      rows.push({
        infinitive,
...
describe('swedish_verbs.csv - issue #125 naive-template conjugation audit (PR #158)', () => {
  ...
  function parseCsv(): CsvRow[] {
    ...
    return lines.slice(1).map((line) => {
      ...
      // noUncheckedIndexedAccess; a short row yields '' fields rather than
      // undefined so CsvRow stays all-string.
      const [
        cefr = '',
        grammar = '',
        infinitive = '',
