...
describe("PracticeCard - lang='sv' on inline Swedish word display (issue #112 AC #5)", () => {
  it("marks the pattern heading and letter-tile buttons with lang='sv' (typing mode)", async () => {
    renderWithProviders(
      <PracticeCard
        infinitive="vara"
        form="presens"
    ...
    // Letter tiles are rendered once shuffledLetters resolves.
    await waitFor(() => {
      expect(screen.getAllByRole('button', { name: /^[a-zäöå]$/i }).length).toBeGreaterThan(0);
    });
    const letterButton = screen.getAllByRole('button', { name: /^[a-zäöå]$/i })[0];
    const letterSpan = letterButton.querySelector('span');
    expect(letterSpan).toHaveAttribute('lang', 'sv');
  });
