...
describe('PracticeCard - empty imperativ', () => {
  ...
  // Regression for issue #106 / PR #127: multiple-choice generation now
  // explicitly skips building options at all when the target form has no
  // data (`correctAnswer !== NOT_AVAILABLE` guard), instead of offering
  // "(not available)" as a clickable, gradeable option.
  it('multiple-choice mode: renders zero options rather than offering "(not available)" as a choice', async () => {
    renderWithProviders(
      <PracticeCard
        infinitive="kunna"
        form="imperativ"
    ...
    // Let the component finish loading (pattern/conjugation resolve).
    await screen.findByRole('heading', { level: 2 });

    // No option-generation should ever run for a form with no data, so no
    // buttons of any kind should appear (this mode has no typing/hint UI).
