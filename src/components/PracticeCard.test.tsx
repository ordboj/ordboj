...
describe('PracticeCard - typing mode', () => {
  ...
  it('calls onAnswer with {correct: true, hintsUsed: 0} for a correct answer and {correct: false, hintsUsed: 0} for a wrong one (issue #30 payload shape)', async () => {
    const user = userEvent.setup();
    const onAnswer = vi.fn<(result: AnswerResult) => void>();
    renderWithProviders(
      <PracticeCard
        infinitive="vara"
        form="presens"
    ...
    expect(onAnswer).toHaveBeenCalledWith({ correct: true, hintsUsed: 0 });
  });

  it('calls onAnswer with {correct: false, hintsUsed: 0} for a wrong answer with no hints used', async () => {
    const user = userEvent.setup();
    const onAnswer = vi.fn<(result: AnswerResult) => void>();
    renderWithProviders(
      <PracticeCard
    ...
    expect(onAnswer).toHaveBeenCalledWith({ correct: false, hintsUsed: 0 });
  });

  it('reports the number of hints actually used to onAnswer, even on a correct answer (a hinted answer is not a perfect recall)', async () => {
    const user = userEvent.setup();
    const onAnswer = vi.fn<(result: AnswerResult) => void>();
    // "gå" preteritum is "gick" (4 letters, cap floor(4/2)=2). Which index
    // gets revealed is random; only the count matters here.
    renderWithProviders(
      <PracticeCard
        infinitive="gå"
        form="preteritum"
    ...
    await user.type(input, 'gick');
    await screen.findByText('Correct!');
    await user.click(screen.getByRole('button', { name: /next card/i }));

    expect(onAnswer).toHaveBeenCalledWith({ correct: true, hintsUsed: 1 });
  });
});

describe('PracticeCard - hint cap (issue #30: floor(answer.length/2))', () => {
  it('caps hint reveals at floor(answer.length/2) and disables the Hint button once the cap is hit, never revealing the full answer via hints', async () => {
    const user = userEvent.setup();
    // "gå" preteritum "gick": length 4, cap = floor(4/2) = 2.
    renderWithProviders(
      <PracticeCard
        infinitive="gå"
        form="preteritum"
    ...
    const hintButton = await screen.findByRole('button', { name: /hint/i });
    const heading = await screen.findByRole('heading', { level: 2 });
    expect(heading.textContent?.match(/_/g)?.length).toBe(4); // fully blank at the start

    await user.click(hintButton);
    expect(heading.textContent?.match(/_/g)?.length).toBe(3);
    expect(hintButton).not.toBeDisabled();

    await user.click(hintButton);
    // Cap reached: exactly 2 of the 4 letters revealed, 2 blanks remain.
    expect(heading.textContent?.match(/_/g)?.length).toBe(2);
    expect(hintButton).toBeDisabled();

    // A further click (e.g. a stray double-click) must not push past the cap.
    await user.click(hintButton);
    expect(heading.textContent?.match(/_/g)?.length).toBe(2);
  });

  it("a 2-letter answer (vara presens 'är') caps at 1 hint, not full reveal", async () => {
    const user = userEvent.setup();
    renderWithProviders(
      <PracticeCard
        infinitive="vara"
    ...
    // 1 of 2 letters revealed, 1 blank remains -- never the full answer.
    expect(heading.textContent?.match(/_/g)?.length).toBe(1);
  });
});
