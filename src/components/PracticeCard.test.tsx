...
describe('PracticeCard - empty imperativ (issue #124)', () => {
  // "kunna" is a modal verb: VERB_DATA marks it imperativNotApplicable and
  // its imperativ is "". conjugateVerb() no longer maps that to the literal
  // string "(not available)" (issue #124) - it now returns imperativ: "".
  ...
  // itself does not guard against it. Pin what actually happens if it did
  // reach the card directly: it must not crash, and - the regression this
  // guards against - it must never again treat the literal placeholder
  // string "(not available)" as a correct answer.
  it('renders without crashing and never accepts the old placeholder string as a correct answer', async () => {
    renderWithProviders(
      <PracticeCard
        infinitive="kunna"
        form="imperativ"
    ...
    const user = userEvent.setup();
    // Check Answer must stay disabled while the input is empty - there is no
    // real answer to autofill or accept for a form with an empty imperativ.
    expect(screen.getByRole('button', { name: /check answer/i })).toBeDisabled();

    // Regression pin for the pre-#124 bug: typing the literal placeholder
    // string used to be graded "Correct!" because conjugateVerb() mapped
    // the missing imperativ to that exact string. It must not be anymore.
