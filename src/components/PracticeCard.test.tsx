...
describe('PracticeCard - wrong-answer feedback shows submitted input (#136)', () => {
  it("typing mode: shows the learner's own wrong input next to the correct answer", async () => {
    const user = userEvent.setup();
    renderWithProviders(
      <PracticeCard
        infinitive="vara"
    ...
    // The learner's own submission is shown verbatim (not lowercased, not
  ...
  it('typing mode: preserves the exact case of the wrong submission (comparison is case-insensitive, display is not)', async () => {
    const user = userEvent.setup();
    renderWithProviders(
      <PracticeCard
        infinitive="vara"
    ...
    const input = await screen.findByPlaceholderText('Type your answer...');
    await user.type(input, 'ARSNULL');
    await user.click(screen.getByRole('button', { name: /check answer/i }));

    await screen.findByText('Not quite');
    // Displayed exactly as typed, uppercase preserved.
    expect(screen.getByText('ARSNULL')).toBeInTheDocument();
    expect(screen.queryByText('arsnull')).not.toBeInTheDocument();
  });

  it('multiple-choice mode: shows the wrong option the learner clicked as their submitted input', async () => {
    renderWithProviders(
      <PracticeCard
        infinitive="vara"
        form="presens"
    ...
    await screen.findByText('Not quite');
    expect(screen.getByText('You wrote')).toBeInTheDocument();
    // The option buttons are hidden once feedback is shown, so any match on
    // the clicked option's text must be the new "You wrote" feedback copy.
    expect(screen.getByText(wrongText)).toBeInTheDocument();
  });

  it("typing mode: 'You wrote' clears when the card advances to a new form (does not leak the previous card's wrong answer)", async () => {
    const user = userEvent.setup();
    ...
    const input = await screen.findByPlaceholderText('Type your answer...');
    await user.type(input, 'firstwrongone');
    await user.click(screen.getByRole('button', { name: /check answer/i }));
    await screen.findByText('firstwrongone');

    // Simulate Practice.tsx advancing to the next card: a new form for the
    // same infinitive is a fresh card with fresh local state.
    rerender(
      <PracticeCard
        infinitive="vara"
        form="preteritum"
    ...
    expect(screen.queryByText('firstwrongone')).not.toBeInTheDocument();
