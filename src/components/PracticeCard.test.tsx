...
describe('PracticeCard - typing mode', () => {
  ...
  it('grades a correct answer 3 (not 5) once a hint was used, per the graded-mapping decision (docs/learning/lapse-handling.md)', async () => {
    const user = userEvent.setup();
    const onAnswer = vi.fn<(grade: Grade) => void>();
    renderWithProviders(
      <PracticeCard
  ...
  it('still grades a wrong answer 0 even if a hint was used first', async () => {
    const user = userEvent.setup();
    const onAnswer = vi.fn<(grade: Grade) => void>();
    renderWithProviders(
      <PracticeCard
