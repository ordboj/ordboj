...
describe('PracticeCard - typing mode', () => {
  ...
  it('calls onAnswer with grade 3 (not 5) when a hint was revealed before answering correctly', async () => {
    const user = userEvent.setup();
    const onAnswer = vi.fn<(grade: Grade) => void>();
    renderWithProviders(
      <PracticeCard
  ...
  it('still calls onAnswer with grade 0 for a wrong answer, even after a hint was revealed (a hint never turns a lapse into partial credit)', async () => {
    const user = userEvent.setup();
    const onAnswer = vi.fn<(grade: Grade) => void>();
    renderWithProviders(
      <PracticeCard
  ...
  it('shows a same-session relearning message (not a next-day one) on a wrong answer', async () => {
    const user = userEvent.setup();
    renderWithProviders(
      <PracticeCard
        infinitive="vara"
    ...
    expect(await screen.findByText(/come back later in this session/i)).toBeInTheDocument();
  });
});
