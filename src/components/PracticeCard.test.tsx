...
describe('PracticeCard - empty imperativ', () => {
  // "kunna" is a modal verb explicitly flagged `noImperativ: true` in
  // VERB_DATA (issue #124): it has no natural imperativ in Swedish, so its
  // imperativ field is the empty string and conjugateVerb() no longer
  // dresses that up with the removed "(not available)" placeholder.
  // useSrsProgress.getDueItems() filters this item out of every real
  // practice session (falsy-form skip), so a learner never reaches this
  // screen for "kunna" + imperativ in normal use - but PracticeCard itself
  // does not guard against it, so pin what it actually renders if reached
  // directly (e.g. a future caller that bypasses getDueItems).
  it('renders without crashing, never shows the removed placeholder, and never marks a real typed guess correct against the empty target', async () => {
    renderWithProviders(
      <PracticeCard
        infinitive="kunna"
        form="imperativ"
    ...
    const user = userEvent.setup();
    await user.type(screen.getByPlaceholderText('Type your answer...'), 'kunna');
    expect(screen.queryByText('Correct!')).not.toBeInTheDocument();

    await user.click(screen.getByRole('button', { name: /check answer/i }));
    expect(await screen.findByText('Not quite')).toBeInTheDocument();
    // The revealed "correct" answer is the empty string, not the removed
    // placeholder string.
