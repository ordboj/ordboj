...
const mocks = vi.hoisted(() => {
  return {
    ...
    // What answeredToday starts at on mount -- an earlier sitting today.
    answeredTodayInitial: 0,
  };
});
...
// every render too, re-firing the effect that keys on it in a loop. Tests
// that need a different dailyGoal MUTATE this object before rendering
// (beforeEach restores it), which changes the value without changing the
// identity -- exactly what the real hook does on load.
...
vi.mock('@/hooks/useSrsProgress', () => ({
  useSrsProgress: () => {
    // Reactive answeredToday, mirroring the real hook's contract (issue
    // #26): +1 per recorded answer, starting from whatever an earlier
    // sitting today already recorded (mocks.answeredTodayInitial).
    const [answeredToday, setAnsweredToday] = useState(mocks.answeredTodayInitial);
    return {
...
beforeEach(() => {
  ...
  mocks.answeredTodayInitial = 0;
  STABLE_SETTINGS.dailyGoal = 12;
  mocks.dueItems = [
...
describe('Practice page - one full session', () => {
  ...
  it('issue #26: skips fetching a queue at all when answeredToday already meets dailyGoal on mount', async () => {
    // An earlier sitting today already spent the whole goal.
    STABLE_SETTINGS.dailyGoal = 5;
    mocks.answeredTodayInitial = 5;
    renderWithProviders(<Practice />, { route: '/practice' });
  ...
  it('issue #26: dailyGoal larger than the due queue still ends on the completion screen, never a blank page', async () => {
    // Regression: with goal 12 and only 2 due cards, answering the last
    // card used to leave the page on the `items[currentIndex]` null guard
    // (blank screen) instead of the completion screen.
    STABLE_SETTINGS.dailyGoal = 12;
    const user = userEvent.setup();
    renderWithProviders(<Practice />, { route: '/practice' });

    for (const answer of ['är', 'var']) {
      const input = await screen.findByPlaceholderText('Type your answer...');
    ...
    // Queue exhausted with the goal (12) unmet at 2 answers: completion
...
describe('Practice page - regression #103 (mid-session deck reshuffle)', () => {
  it('keeps the deck fixed for the whole session: every item seen exactly once, none skipped or repeated', async () => {
    ...
    for (let i = 0; i < expectedAnswers.length; i++) {
      // Header shows answeredToday / dailyGoal (issue #26): i answers so
      // far, goal steady at 12. The reshuffle regression itself is caught
      // by the answeredOrder assertions below -- a skipped card would
      // break both the order and the total.
      expect(await screen.findByText(`${i} / 12`)).toBeInTheDocument();
