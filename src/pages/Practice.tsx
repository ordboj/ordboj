...
export default function Practice() {
  ...
  const { getDueItems, recordAnswer, srsStates, answeredToday, isLoading } = useSrsProgress(
    settings.cefrLevels,
  );
  ...
  // Same latest-ref treatment for the daily counter and goal: the one-shot
  // load effect below needs their values at load time, but neither may be a
  // dependency (answeredToday changes on every answer).
  const answeredTodayRef = useRef(answeredToday);
  useEffect(() => {
    answeredTodayRef.current = answeredToday;
  }, [answeredToday]);
  const dailyGoalRef = useRef(settings.dailyGoal);
  useEffect(() => {
    dailyGoalRef.current = settings.dailyGoal;
  }, [settings.dailyGoal]);
  ...
  useEffect(() => {
    ...
    const loadDueItems = async () => {
      try {
        ...
        // serve the due deck again, go straight to the completion screen
        // (which still offers extra reviews and free practice). Read via
        // refs so the daily counter is NOT an effect dependency -- it
        // changes on every answer, and re-running this effect mid-session
        // is exactly the desync class #122 fixed.
        if (answeredTodayRef.current >= dailyGoalRef.current) {
  ...
  const handleAnswer = (grade: Grade) => {
    ...
    // The daily goal bounds only the due session. A free round records
    // nothing (it must not burn goal progress), and an extra-review round
    // is an explicit opt-in offered at/after the goal, so bounding it by
    // the goal would end it on its first card. answeredToday hasn't
    // re-rendered with this answer yet, so account for it explicitly
    // rather than reading a stale value.
    const reachedDailyGoal = sessionKind === 'due' && answeredToday + 1 >= settings.dailyGoal;
    if (reachedDailyGoal || currentIndex >= items.length - 1) {
      setSessionComplete(true);
    } else {
  ...
  // The due session's header and bar show progress toward the daily goal
  // (docs/learning/session-shape-and-daily-goal.md); free and extra rounds
  // are outside the goal, so they show position within the round instead.
  const progressPercent =
    sessionKind === 'due'
      ? settings.dailyGoal > 0
        ? Math.min(100, (answeredToday / settings.dailyGoal) * 100)
        : 100
      : items.length > 0
