...
describe('Settings page - issue #93: reset confirmation reaches the real reset path', () => {
  it('closes the dialog and empties the SRS store (version envelope, no items) after confirming, without touching the unrelated settings key', async () => {
    ...
    // The exact set of localStorage keys present after reset: settings is
    // untouched, and the only addition is the daily answer counter, which
    // reset zeroes for today (issue #26: resetProgress resets answeredToday
    // along with the schedule). No other key appears or disappears.
    expect(storedKeys()).toEqual([DAILY_COUNT_KEY, SETTINGS_KEY, SRS_KEY].sort());
    expect(localStorage.getItem(SETTINGS_KEY)).toBe(JSON.stringify({ practiceMode: 'typing' }));
    const dailyCount = JSON.parse(localStorage.getItem(DAILY_COUNT_KEY) as string);
    expect(dailyCount.count).toBe(0);
  });
});
