...
describe('selectDistractors (P14 policy helper)', () => {
  const candidate = (
    infinitive: string,
    conjugationGroup: string,
    value: string,
  ): DistractorCandidate => ({ infinitive, conjugationGroup, value });

  it('never includes a placeholder value, however dominant it is in the pool', () => {
    const candidates: DistractorCandidate[] = [
      candidate('a', '1', '(not available)'),
      candidate('b', '1', '(not available)'),
      candidate('c', '1', '(not available)'),
      candidate('d', '1', 'riktig'),
    ];

    const result = selectDistractors({
      targetInfinitive: 'target',
      targetGroup: '1',
      targetForms: { presens: 'svarar' },
      candidates,
    });

    expect(result).toEqual(['riktig']);
    expect(result).not.toContain('(not available)');
  });

  it("never returns the target verb's own infinitive as a source, even if it appears in the candidate pool", () => {
    const candidates: DistractorCandidate[] = [
      candidate('target', '1', 'sjalv-varde'),
      candidate('other', '1', 'annat-varde'),
    ];

    const result = selectDistractors({
      targetInfinitive: 'target',
      targetGroup: '1',
      targetForms: { presens: 'correct' },
      candidates,
    });

    expect(result).toEqual(['annat-varde']);
  });

  it('rejects a candidate value that is secretly a correct form of the target verb in a different slot (P14)', () => {
    // "target" has presens "X" (the tested form) and preteritum "Y". A
    // distractor whose value is "Y" would be a defensible correct answer,
    // not a wrong one, so it must never be offered.
    const candidates: DistractorCandidate[] = [
      candidate('other1', '1', 'Y'),
      candidate('other2', '1', 'Z'),
    ];

    const result = selectDistractors({
      targetInfinitive: 'target',
      targetGroup: '1',
      targetForms: { presens: 'X', preteritum: 'Y' },
      candidates,
    });
  ...
  it('restricts distractors to the same conjugation group by default (strictest P14 reading)', () => {
    const candidates: DistractorCandidate[] = [
      candidate('other1', '2a', 'fel-grupp'),
      candidate('other2', '1', 'ratt-grupp'),
    ];
    ...
    expect(result).toEqual(['ratt-grupp']);
  });

  it('accepts a caller-supplied adjacency predicate once one exists, without changing the filtering contract', () => {
    const candidates: DistractorCandidate[] = [
      candidate('other1', '2a', 'adjacent-grupp'),
      candidate('other2', '3', 'for-langt-bort'),
    ];
    ...
    const result = selectDistractors({
      ...
      targetForms: { presens: 'correct' },
      candidates,
      isSameOrAdjacentGroup: (target, group) => target === '1' && group === '2a',
    });

    expect(result).toEqual(['adjacent-grupp']);
  });

  it('dedupes identical distractor values from different candidate verbs', () => {
    const candidates: DistractorCandidate[] = [
      candidate('other1', '1', 'samma'),
      candidate('other2', '1', 'samma'),
      candidate('other3', '1', 'annat'),
    ];
    ...
    expect(result).toEqual(expect.arrayContaining(['samma', 'annat']));
    expect(result).toHaveLength(2);
  });

  it('is bounded: returns fewer than poolSize rather than hanging or erroring when too few valid candidates exist', () => {
    const candidates: DistractorCandidate[] = [
      candidate('other1', '1', '(not available)'),
      candidate('other2', '1', '(not available)'),
      candidate('other3', '1', 'enda-giltiga'),
    ];
    ...
    expect(result).toEqual(['enda-giltiga']);
    expect(result.length).toBeLessThan(3);
  });

  it('stops at poolSize even when more valid candidates are available', () => {
    const candidates: DistractorCandidate[] = Array.from({ length: 20 }, (_, i) =>
      candidate(`v${i}`, '1', `varde-${i}`),
    );
  ...
  it("shuffles via the unbiased Fisher-Yates algorithm, matching useSrsProgress.ts's shuffle exactly", () => {
    // With Math.random pinned to 0, Fisher-Yates on [v0,v1,v2,v3] always
    // swaps the current index with index 0, producing a specific, computable
    // permutation. A biased `sort(() => Math.random() - 0.5)` shuffle does
    // not consume Math.random this way and would not reproduce this order.
    const candidates: DistractorCandidate[] = [
      candidate('v0', '1', 'A'),
      candidate('v1', '1', 'B'),
      candidate('v2', '1', 'C'),
    ...
    // Hand-computed Fisher-Yates trace for [A,B,C,D] with Math.random() === 0
    // always (j = 0 at every step):
