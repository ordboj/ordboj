...
// Forms VERB_DATA has no value for (e.g. imperativ on modal verbs like
// "kunna") come through as "" rather than a display sentinel — a missing
// form is a data fact, not a string to quiz on. Callers that surface these
// values to a learner must check availableForms() first.
...
// All quizzable forms, in a stable pedagogical order.
const ALL_FORMS: Form[] = ['infinitive', 'presens', 'preteritum', 'supinum', 'imperativ'];

// Which forms actually exist for a verb. 'infinitive' always exists (it is
// the lookup key). The other four exist only when VERB_DATA stores a
// non-empty string for them: modal/auxiliary verbs (kunna, få, vilja, ...)
// have no imperativ, and this must never be presented as a quizzable form.
// Accepts either an infinitive string or an already-conjugated verb, since
// callers may hold either.
export function availableForms(verb: string | ConjugatedVerb): Form[] {
  const conjugated = typeof verb === 'string' ? VERB_DATA.find((v) => v.infinitive === verb) : verb;

  if (!conjugated) {
    return ['infinitive'];
  }

  return ALL_FORMS.filter((form) => form === 'infinitive' || Boolean(conjugated[form]));
}
