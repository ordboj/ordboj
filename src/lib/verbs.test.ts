...
describe("getVerbs - id scheme", () => {
  it('assigns ids as String(index + 1), matching VERB_DATA order', async () => {
    ...
    expect(verbs[0]).toEqual({ id: "1", infinitive: VERB_DATA[0].infinitive, cefr: VERB_DATA[0].cefr });
    expect(verbs[1]).toEqual({ id: "2", infinitive: VERB_DATA[1].infinitive, cefr: VERB_DATA[1].cefr });
  ...
  it("documents that ids are unstable under reordering of VERB_DATA (fragility, not a passing contract)", async () => {
    ...
    vi.doMock("@/data/verbData", () => ({
      VERB_DATA: [
        { cefr: "A1", infinitive: "alpha", imperativ: "a", presens: "a", preteritum: "a", supinum: "a" },
        { cefr: "A1", infinitive: "beta", imperativ: "b", presens: "b", preteritum: "b", supinum: "b" },
      ],
    }));
